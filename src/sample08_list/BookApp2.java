...
public class BookApp2 {
	...
	public static void main(String[] args) {
		...
		books.add(new Book(102, "Do it! 점프 투 파이썬", "박응용", 35000));
		books.add(new Book(100, "자바의 정석", "남궁성", 35000));
		books.add(new Book(102, "혼자 공부하는 파이썬", "윤인성", 21000));
		books.add(new Book(102, "모던 자바스크립트", "이웅모", 42000));
		books.add(new Book(101, "이것이 자바다", "신용권", 32000));
	
		// List객체에 저장된 책정보 정렬시키기
		Collections.sort(books);						// 번호순으로 정렬되어있다.
		
		// List객체에 저장된 책정보를 가격 기준으로 정렬시키기
		Collections.sort(books, (b1, b2) -> b1.getPrice() - b2.getPrice());
		
		// List객체에 저장된 책정보를 저자명을 가준으로 정렬시키기
