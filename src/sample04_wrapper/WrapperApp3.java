...
public class WrapperApp3 {
	...
	public static void main(String[] args) {
		...
		// Wrapper 클래스의 주요 API//pars정적메소드
		
		// int Integer.parseInt(String text) : 숫자로구성된 문자열을 정수로 변환해서 반환한다.  
		int num1 = Integer.parseInt("234");
		int num2 = Integer.parseInt("12");			//NumberFormatException(많이 접하게 될 오류)- 숫자가 아닌 다른값이 있어서 오류 숫자로 구성된 문자열
		System.out.println(num1*num2);
		
		// int Integer.parseInt(String text, int radix)	: 숫자로 구성된 문자열을 지정된 진법으로 환산해서 정수로 변환한다음 반환한다.
		int num3 = Integer.parseInt("1001", 2);		// 1*2^3 + 0*2^2 + 0*2^1 + 0*2^0  = 8 + 0 + 0 + 1 = 9
		System.out.println(num3);
		int num4 = Integer.parseInt("1a", 16);		// 0*16^1 +10*16^0 = 16 + 10 = 26
