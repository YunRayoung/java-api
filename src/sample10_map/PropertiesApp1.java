...
public class PropertiesApp1 {
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * - Properties객체는 Key와 Value의 타입이 모드 String 타입이다.
		 * - 주요 API
		 * 		void load(FileInPutStream in)
		 * 			- 지정된 FileInPutStream 으로 properties 파일 정보를 로딩한다.
		 * 		void load(FileReader in)
		 * 			- 지정된 FileReader 으로 properties 파일 정보를 로딩한다.
		 * 		void loadFromXML(FileInPutStream in)
		 * 			- 지정된 FileInPutStream 으로 xml 파일 정보를 로딩한다.
		 * 		void loadFromXML(FileReader in)
		 * 			- 지정된 FileReader 으로 xml 파일 정보를 로딩한다.
